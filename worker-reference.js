/**
 * Portfolio Data Fetcher Worker
 * 
 * This is a reference implementation for your existing worker.
 * Copy this code into your deployed worker (daily-goodreads-letterboxd-datafetcher).
 * 
 * Features:
 * - Fetches Goodreads RSS and stores in KV
 * - Fetches Letterboxd RSS, scrapes each film page for poster images, stores in KV
 * - Triggers Cloudflare Pages deploy hook to rebuild the site
 * - Cron schedule: daily at midnight Lisbon time
 */
...
async function fetchGoodreads(env) {
  ...
  await env.PORTFOLIO_KV.put('goodreads', JSON.stringify(data), {
...
async function fetchLetterboxd(env) {
  ...
  const data = { films };
  await env.PORTFOLIO_KV.put('letterboxd', JSON.stringify(data), {
    expirationTtl: 172800, // 2 days
  });
